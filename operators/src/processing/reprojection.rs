...
impl Reprojection {
    fn derive_raster_result_descriptor(
        ...
        let out_bbox = if let Some(bbox) = in_desc.bbox {
        ...
        let out_res = match (in_desc.bbox, in_desc.resolution, out_bbox) {
            (Some(in_bbox), Some(in_res), Some(out_bbox)) => {
