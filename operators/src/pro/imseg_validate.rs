...
use ndarray::{Array2, ArrayBase, ViewRepr, Dim};
    ...
    C: 'static,
{
    ...
    let mut counts: Vec<Vec<usize>> = vec![vec![0,0,0,0,0], vec![0,0,0,0,0], vec![0,0,0,0,0], vec![0,0,0,0,0], vec![0,0,0,0,0]];
        ...
        //let x = bffr[0].next().await;
            ...
            while let (Some(proc), Some(truth)) = (zip.next().await, truth_stream.next().await) {
                ...
                if final_buff_proc.len() == batch_size {
                    ...
                    let mut arr_truth_batch = ndarray::Array::from_elem((batch_size, tile_size[0], tile_size[1],1), no_data_value);
                    ...
                    for j in 0..batch_size {
                        ...
                        arr_truth_batch.slice_mut(ndarray::s![j,..,..,0]).assign(&arr_t);
                    ...
                    let result_img = py_mod.call("predict", (py_img, batch_size), None)
                    .unwrap()
                    ...
                    for i in 0..batch_size {
                        ...
                        let truth: ArrayBase<ViewRepr<&u8>, Dim<[usize; 2]>> = arr_truth_batch.slice(ndarray::s![i,..,..,0]);
...
mod tests {
    ...
    async fn imseg_fit_test() {
        ...
        let x = imseg_fit(vec![proc_ir_039, proc_ir_087, proc_ir_097, proc_ir_108, proc_ir_120, proc_ir_134],proc_claas, QueryRectangle {
            ...
            time_interval: TimeInterval::new(1199167200000, 1230703200000)
                .unwrap(),
