...
async fn layer_to_dataset<C: Context>(
    ...
    let db = ctx.db(session.clone().into());
...
async fn add_layer<C: Context>(
    session: AdminSession, // TODO: allow normal users to add layers to their stuff
    ctx: web::Data<C>,
...
async fn add_collection<C: Context>(
    session: AdminSession, // TODO: allow normal users to add collections to their stuff
    ctx: web::Data<C>,
    collection: web::Path<LayerCollectionId>,
...
async fn remove_collection<C: Context>(
    session: AdminSession, // TODO: allow normal users to remove their collections
    ctx: web::Data<C>,
    collection: web::Path<LayerCollectionId>,
) -> Result<HttpResponse> {
    ctx.db(session.into())
