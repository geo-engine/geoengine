...
pub async fn add_to_current_schema(tx: &Transaction<'_>) -> Result<()> {
...
pub async fn populate_current_schema(
    ...
    tx.execute(
        ...
        &[
            ...
            &bcrypt::hash(user_config.admin_password).expect("Admin password hash should be valid"),
            &ADMIN_QUOTA,
        ],
    )
    .await?;
...
impl ProMigration for ProMigrationImpl<CurrentSchemaMigration> {
    async fn pro_migrate(&self, tx: &Transaction<'_>) -> Result<()> {
        ...
        populate_current_schema(tx, user_config).await
    }
}
