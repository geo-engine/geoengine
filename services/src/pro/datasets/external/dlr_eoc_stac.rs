...
use super::{GdalRetries, StacApiRetries};
...
pub struct DlrEocStacProviderDefinition {
...
#[async_trait]
impl<D: GeoEngineDb> DataProviderDefinition<D> for DlrEocStacProviderDefinition {
    async fn initialize(self: Box<Self>, _db: D) -> crate::error::Result<Box<dyn DataProvider>> {
        Ok(Box::new(DlrEocStacDataProvider::new(
            self.id,
            self.name,
            self.description,
...
#[derive(Debug)]
pub struct DlrEocStacDataProvider {
    id: DataProviderId,
...
pub enum DlrEocStacDataProviderError {
    #[snafu(display("Error while converting STAC data provider collection id: {source}"))]
    CollectionId { source: serde_json::Error },
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", tag = "type")]
pub enum DlrEocStacDataProviderCollectionId {
    Collections,
    Collection { collection: String },
    Asset { collection: String, asset: String },
}

impl TryFrom<&DlrEocStacDataProviderCollectionId> for LayerCollectionId {
    type Error = DlrEocStacDataProviderError;

    fn try_from(value: &DlrEocStacDataProviderCollectionId) -> Result<Self, Self::Error> {
        let json = serde_json::to_string(value).context(error::CollectionId)?;
        Ok(LayerCollectionId(json))
    }
}
...
impl TryFrom<DlrEocStacDataProviderCollectionId> for LayerCollectionId {
    ...
    fn try_from(value: DlrEocStacDataProviderCollectionId) -> Result<Self, Self::Error> {
        LayerCollectionId::try_from(&value)
    }
}

impl TryFrom<&LayerCollectionId> for DlrEocStacDataProviderCollectionId {
    type Error = DlrEocStacDataProviderError;

    fn try_from(value: &LayerCollectionId) -> Result<Self, Self::Error> {
        let id = serde_json::from_str(&value.0).context(error::CollectionId)?;
        Ok(id)
    }
}
...
impl TryFrom<LayerCollectionId> for DlrEocStacDataProviderCollectionId {
    ...
    fn try_from(value: LayerCollectionId) -> Result<Self, Self::Error> {
        DlrEocStacDataProviderCollectionId::try_from(&value)
    }
}

// struct DlrEocStacDataset {
//     collection: String,
//     asset: String,
//     band: String,
// }

// fn to_layer_id(collection: &str, asset: &str, band: &str) -> LayerId {
//     LayerId(format!("{}:{}:{}", collection, asset, band))
// }

// fn from_layer_id(layer_id: &LayerId) -> Result<(String, String, String)> {
//     let parts: Vec<&str> = layer_id.0.split(':').collect();
//     (parts[0].to_string(), parts[1].to_string(), parts[2].to_string())
// }

impl DlrEocStacDataProvider {
    ...
    pub fn new(
        ...
        Self {
            ...
            description,
            api_url,
            // TODO: query assets for different layers
            // TODO: query bands of asset
            collection_names: vec!["ENMAP_HSI_L2A".into()],
            stac_api_retries,
            gdal_retries,
            cache_ttl,
        }
...
#[async_trait]
impl DataProvider for DlrEocStacDataProvider {
...
#[async_trait]
impl LayerCollectionProvider for DlrEocStacDataProvider {
    ...
    async fn load_layer_collection(
        ...
        match DlrEocStacDataProviderCollectionId::try_from(collection)? {
            DlrEocStacDataProviderCollectionId::Collections => {
                let mut items = self
                    .collection_names
                    .iter()
                    .map(|collection_name| {
                        Ok(CollectionItem::Collection(LayerCollectionListing {
                            id: ProviderLayerCollectionId {
                                provider_id: self.id,
                                collection_id: DlrEocStacDataProviderCollectionId::Collection {
                ...
                Ok(LayerCollection {
                    ...
                    description: "DlrEocStac".to_owned(),
                    items,
                    entry_label: None,
                    properties: vec![],
                })
            }
            DlrEocStacDataProviderCollectionId::Collection { collection } => {
                // Assumption: all collections have the same assets
                // Thus, we can just load the first feature of a collection

                // TODO: add retry to web request

                let collection = DlrEocStacMetaData {
                    api_url: self
                        .api_url
                ...
                .load_collection(&Vec::<(String, String)>::new(), 0)
                .await?;

                for (asset_name, asset) in collection.features.first().map_or_else(
                    || itertools::Either::Left(std::iter::empty()),
                    |feature| itertools::Either::Right(feature.assets.iter()),
                ) {
                    if !asset.mime_type.starts_with("image/tiff") {
                        continue;
                    }
            ...
            DlrEocStacDataProviderCollectionId::Asset { collection, asset } => todo!(),
        }
    ...
    async fn get_root_layer_collection_id(&self) -> Result<LayerCollectionId> {
        Ok(DlrEocStacDataProviderCollectionId::Collections.try_into()?)
    }

    async fn load_layer(&self, id: &LayerId) -> Result<Layer> {
        // let dataset = self.collection_names.get(id).ok_or(Error::UnknownDataId)?;
...
mod tests {
    ...
    async fn it_displays_collections(app_ctx: ProPostgresContext<NoTls>) {
        // TODO: mock STAC endpoint

        let provider_id = DataProviderId::from_u128(0x592cf760_42a9_49ea_90ad_e1dff3ab01bf);

        let provider_definition = DlrEocStacProviderDefinition {
            id: provider_id,
            name: "EnMap Data".into(),
            description: "EnMap Data".into(),
            priority: None,
            api_url: "https://geoservice.dlr.de/eoc/ogc/stac/v1/".into(),
            stac_api_retries: StacApiRetries {
                number_of_retries: 3,
            ...
            // TODO: remove username and password from definition
            username: "christian.beilschmidt-enmap".into(),
            password: "fkN8lAWVNtv4e4SfNZ8m".into(),
        };

        let provider = Box::new(provider_definition)
            .initialize(
                app_ctx
        ...
        let root_collection_id = provider.get_root_layer_collection_id().await.unwrap();
        let root_collection = provider
        ...
        pretty_assertions::assert_eq!(
            ...
            LayerCollection {
                id: ProviderLayerCollectionId {
                    provider_id,
                    collection_id: DlrEocStacDataProviderCollectionId::Collections
                        .try_into()
                        .unwrap(),
                },
                name: "Element 84 AWS STAC".to_owned(),
                ...
                items: vec![CollectionItem::Collection(LayerCollectionListing {
                    id: ProviderLayerCollectionId {
                        provider_id,
                        collection_id: DlrEocStacDataProviderCollectionId::Collection {
                            collection: "ENMAP_HSI_L2A".into()
                        }
                        .try_into()
                        .unwrap(),
                    },
                    name: "ENMAP_HSI_L2A".into(),
        ...
        let assets_collection = provider
            .load_layer_collection(
                &DlrEocStacDataProviderCollectionId::Collection {
                    collection: "ENMAP_HSI_L2A".into(),
                }
                .try_into()
                .unwrap(),
                LayerCollectionListOptions::default(),
            )
            .await
            .unwrap();
    ...
    //     let def: DlrEocStacProviderDefinition = serde_json::from_reader(BufReader::new(
    ...
    //         Box::new(DlrEocStacProviderDefinition {
    ...
    //     let uts = DlrEocStacMetaData::make_unique_start_times_from_sorted_features(&timestamps);
