...
static SENTINEL_2_COLLECTION_NAME: &str = "SENTINEL-2";
static L2A_PRODUCT_TYPE: &str = "S2MSI2A";
static EODATA_RELATIVE_PATH: &str = "Sentinel-2/MSI/L2A";
static IMAGE_FILENAME: &str = "MTD_MSIL2A.xml";

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CreodiasSentinelS2L2AProviderDefinition {
    name: String,
    id: DataProviderId,
    api_url: String,
...
#[typetag::serde]
#[async_trait]
impl DataProviderDefinition for CreodiasSentinelS2L2AProviderDefinition {
    async fn initialize(self: Box<Self>) -> crate::error::Result<Box<dyn DataProvider>> {
        Ok(Box::new(CreodiasSentinelS2L2ADataProvider::new(
            self.id,
            self.api_url,
...
#[derive(Debug)]
pub struct CreodiasSentinelS2L2ADataProvider {
    id: DataProviderId,
...
impl CreodiasSentinelS2L2ADataProvider {
    pub fn new(
        id: DataProviderId,
        api_url: String,
        volume_mount_path: PathBuf,
            ...
            .flat_map(|zone| {
                bands.iter().map(move |band| {
                    ...
                    let listing = Layer {
                        ...
                        name: format!("Creodias Sentinel S2 L2A {}:{}", zone.name, band.name),
...
#[async_trait]
impl DataProvider for CreodiasSentinelS2L2ADataProvider {
...
#[async_trait]
impl LayerCollectionProvider for CreodiasSentinelS2L2ADataProvider {
    async fn load_layer_collection(
        ...
        Ok(LayerCollection {
            ...
            name: "Creodias Datahub Sentinel-2 L2A".to_owned(),
            description: "CreodiasSentinelS2L2A".to_owned(),
            items,
            entry_label: None,
            properties: vec![],
        })
    ...
    async fn get_root_layer_collection_id(&self) -> Result<LayerCollectionId> {
        Ok(LayerCollectionId("CreodiasSentinelS2L2A".to_owned()))
    }
...
#[derive(Debug, Clone)]
pub struct CreodiasSentinelS2L2AMetaData {
    api_url: String,
    volume_mount_path: PathBuf,
    zone: Zone,
    band: Band,
...
impl CreodiasSentinelS2L2AMetaData {
    async fn create_loading_info(&self, query: RasterQueryRectangle) -> Result<GdalLoadingInfo> {
        ...
        for i in 0..num_features {
            ...
            if time_interval.intersects(&query.time_interval) {
                ...
                parts.push(self.create_loading_info_part(time_interval, feature)?);
            }
        }
    ...
    fn create_loading_info_part(
        ...
        feature: &crate::stac::Feature,
    ) -> Result<GdalLoadingInfoTemporalSlice> {
        // let [stac_shape_y, stac_shape_x] = asset.proj_shape.ok_or(error::Error::StacInvalidBbox)?;
        // asume that the start time corresponds to the date of the image, as that determnes the file name in the eodata volume
        let time = time_interval
            .start()
            .as_date_time()
            .ok_or(error::Error::StacInvalidTime)?;

        // create the file path from the settings and the infos provided by STAC
        let file_path = self.volume_mount_path.join(&format!(
            "{EODATA_RELATIVE_PATH}/{year}/{month}/{day}/{id}/{file_name}",
            year = time.year(),
            month = time.month(),
            day = time.day(),
            id = feature.id,
        ...
        // as the Creodias STAC does not include the infos about shape and geo transform we have to read them from the file
        let dataset = gdal_open_dataset(&file_path)?;

        // TODO: derive sub dataset that includes the requested band
        // dataset.subdatasets()
        // dataset.metadata_item(key, domain)
    ...
    fn request_params(&self, query: RasterQueryRectangle) -> Result<Option<Vec<(String, String)>>> {
        ...
        Ok(bbox.map(|bbox| {
            vec![
                (
                    "collection".to_owned(),
                    SENTINEL_2_COLLECTION_NAME.to_owned(),
                ),
                (
                    "bbox".to_owned(), // has to always be WGS84 for Creodias STAC
                    format!(
                        "{},{},{},{}",
                        bbox.lower_left().x,
                        bbox.lower_left().y,
    ...
    async fn load_all_features<T: Serialize + ?Sized + Debug>(
        ...
        // load all features at once because the Creodias STAC does not support pagination
        let mut collection = self.load_collection_from_stac(params).await?;

        // filter the features by product type because the Creodias STAC does not support filtering beyond specifying the collection (Sentinel-2)
        collection.features.retain(|feature| {
            feature
                .properties
                .product_type
                .as_ref()
                .map(|p| p == L2A_PRODUCT_TYPE)
                .unwrap_or(false)
        });
    ...
    async fn load_collection_from_stac<T: Serialize + ?Sized + Debug>(
        &self,
        params: &T,
...
impl MetaData<GdalLoadingInfo, RasterResultDescriptor, RasterQueryRectangle>
    for CreodiasSentinelS2L2AMetaData
{
...
impl MetaDataProvider<GdalLoadingInfo, RasterResultDescriptor, RasterQueryRectangle>
    for CreodiasSentinelS2L2ADataProvider
{
    async fn meta_data(
        &self,
    ...
    > {
        ...
        Ok(Box::new(CreodiasSentinelS2L2AMetaData {
...
mod tests {
    ...
    #[test]
    fn test_name() {
        // mount the eodata locally for testing
        // sudo mkdir /eodata
        // sudo chown user:user /eodata
        // sshfs eouser@crophype.app.geoengine.io:/eodata /eodata

        let path = "/eodata/Sentinel-2/MSI/L2A/2023/01/01/S2B_MSIL2A_20230101T224829_N0509_R072_T54CVU_20230101T235704.SAFE/MTD_MSIL2A.xml";

        let dataset = gdal_open_dataset(Path::new(path)).unwrap();
        dbg!(dataset.metadata_domain("SUBDATASETS"));
