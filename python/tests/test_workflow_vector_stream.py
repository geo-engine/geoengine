    ...
    def test_streaming_workflow(self):
        ...
        with unittest.mock.patch("websockets.asyncio.client.connect", return_value=MockWebsocket()):
            ...
            async def inner2():
                ...
                assert pd.notna(data_frame["time_start"]).all()
                assert pd.notna(data_frame["time_end"]).all()

                assert str(data_frame["time_start"][0]) == "-262144-01-01 00:00:00+00:00"
                assert str(data_frame["time_end"][0]) == "262143-12-31 23:59:59.999000+00:00"
