    ...
    CollectionType: Geometry + ArrowTyped,
{
    ...
    fn sort_by_time_asc(&self) -> Result<Self::Output> {
        // TODO: use functions from arrow 3.0 directly

        pub fn as_list_array<S: arrow::array::OffsetSizeTrait>(
            arr: &ArrayRef,
        ) -> &arrow::array::GenericListArray<S> {
            arr.as_any()
                .downcast_ref::<arrow::array::GenericListArray<S>>()
                .expect("Unable to downcast to list array")
        }

        fn cmp_array(a: &dyn Array, b: &dyn Array) -> std::cmp::Ordering {
        ...
        pub type DynComparator<'a> = Box<dyn Fn(usize, usize) -> std::cmp::Ordering + 'a>;

        fn compare_primitives<'a, T: arrow::datatypes::ArrowPrimitiveType>(
            left: &'a dyn Array,
            right: &'a dyn Array,
        ) -> DynComparator<'a>
        where
        ...
        pub fn build_compare<'a>(
            left: &'a dyn Array,
            right: &'a dyn Array,
        ) -> Result<DynComparator<'a>, arrow::error::ArrowError> {
            // use DataType::*;
            // use IntervalUnit::*;
            // use TimeUnit::*;
            Ok(match (left.data_type(), right.data_type()) {
                ...
                (arrow::datatypes::DataType::Int64, arrow::datatypes::DataType::Int64) => {
                    compare_primitives::<Int64Type>(left, right)
                }
        ...
        fn u32_vec_to_array(
            data: Vec<
                <arrow::datatypes::UInt32Type as arrow::datatypes::ArrowPrimitiveType>::Native,
            >,
        ) -> arrow::array::PrimitiveArray<arrow::datatypes::UInt32Type> {
            use arrow::datatypes::ToByteSlice;

            let array_data = ArrayData::builder(arrow::datatypes::DataType::UInt32)
                .len(data.len())
                .add_buffer(arrow::buffer::Buffer::from(data.to_byte_slice()))
                .build();
            arrow::array::PrimitiveArray::from(array_data)
        }
            ...
            T::Native: std::cmp::PartialOrd,
        {
            ...
            if options.nulls_first {
                ...
                // return Ok(arrow::array::UInt32Array::from(null_indices));
                return Ok(u32_vec_to_array(null_indices));
            }

            valid_indices.append(&mut null_indices);
            // Ok(arrow::array::UInt32Array::from(valid_indices))
            Ok(u32_vec_to_array(valid_indices))
        }
        ...
        let (v, n) = partition_validity(time_column);
        let sort_indices = sort_list::<i32, arrow::datatypes::UInt64Type>(
            time_column,
            v,
            n,
            &sort_options.unwrap(),
        ...
        let table_ref: Arc<dyn arrow::array::Array> = make_array(self.table.data());

        let table_ref = arrow::compute::take(&table_ref, &sort_indices, None)?;
