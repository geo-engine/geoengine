...
use geoengine_datatypes::raster::{
    Grid, Grid1D, GridIdx, GridIdx2D, GridShape, MapIndexedElements, MapIndexedElementsParallel,
    MapIndexedElementsParallel2D,
};
...
fn grid_map_2d_indexed_elements_2d_grididx(c: &mut Criterion) {
    ...
    group.bench_function("Grid 2D", |b| {
        b.iter(|| {
            ...
            black_box({
                let map_fn = |GridIdx([y, x]): GridIdx2D, element: u16| {
                    (element * 321 + 1) % (y * 64 + x + 1) as u16
                };
                let _ = grid.map_indexed_elements(map_fn);
            })
        })
...
criterion_group!(
    benches,
    grid_map_linear_indexed_elements_1d,
    grid_map_linear_indexed_elements_2d_usize,
    grid_map_2d_indexed_elements_2d_grididx,
    grid_map_2d_indexed_elements_2d_grididx_selfmade
);
