...
impl GdalHandling {
    ...
    pub fn load_tile_data_with_dataset_retry<T: Pixel + GdalType + FromPrimitive>(
        ...
        retry_sync(
            ...
            || {
                let open_span = tracing::debug_span!(
                    target: "gdalsource-process",
                    "dataset.open",
                    file_path = %dp.file_path.display(),
                )
                .entered();
        ...
        D: Clone + GridSize + PartialEq,
    {
        ...
        let _read_span = tracing::debug_span!(
            target: "gdalsource-process",
            "rasterband.read_as",
            window_x = read_window.gdal_window_start().0,
            window_y = read_window.gdal_window_start().1,
            window_w = read_window.gdal_window_size().0,
            window_h = read_window.gdal_window_size().1,
        )
        .entered();
