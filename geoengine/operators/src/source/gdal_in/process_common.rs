...
pub struct IpcChannelMessagePayload {
    ...
    /// OpenTelemetry span context propagated from parent to child process.
    /// Allows the GDAL subprocess to attach its spans to the correct parent trace.
    #[serde(default)]
    pub span_context: std::collections::HashMap<String, String>,
    /// Whether this request is a dedup-follower (attached to an in-flight leader).
    /// Always `false` for requests that actually reach the GDAL worker;
    /// followers wait on the leader's result via a watch channel.
    #[serde(default)]
