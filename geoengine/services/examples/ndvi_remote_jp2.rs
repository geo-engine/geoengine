...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Set global GDAL config for code-de /vsis3 S3 access.
    gdal::config::set_config_option("AWS_S3_ENDPOINT", CODE_DE_S3_ENDPOINT)?;
    gdal::config::set_config_option("AWS_VIRTUAL_HOSTING", "FALSE")?;
    if let Ok(access_key) = std::env::var("AWS_ACCESS_KEY_ID") {
        gdal::config::set_config_option("AWS_ACCESS_KEY_ID", &access_key)?;
    }
    if let Ok(secret_key) = std::env::var("AWS_SECRET_ACCESS_KEY") {
        gdal::config::set_config_option("AWS_SECRET_ACCESS_KEY", &secret_key)?;
    }
