-- Preserve existing plaintext credentials while replacing their text attributes.
...
SET
    definition.stac_data_provider_definition.s3_config.access_key = convert_to(
        (((definition).stac_data_provider_definition).s3_config).legacy_access_key,
        'UTF8'
    ),
    definition.stac_data_provider_definition.s3_config.secret_key = convert_to(
        (((definition).stac_data_provider_definition).s3_config).legacy_secret_key,
        'UTF8'
    )
WHERE ((definition).stac_data_provider_definition).s3_config IS DISTINCT FROM NULL;
