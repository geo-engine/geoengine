...
impl TimeEstimation {
    ...
    pub fn update(&mut self, time_stamp: DateTime, pct_complete: f64) {
        let pct_complete = pct_complete.clamp(0., 1.);

        let elapsed_secs = (time_stamp - self.initial_time).num_seconds() as f64;
    ...
    pub fn update_now(&mut self, pct_complete: f64) {
        self.update(DateTime::now(), pct_complete);
    }

    pub fn time_total(&self) -> String {
        hms_from_secs((DateTime::now() - self.initial_time).num_seconds() as f64)
    }

    pub fn time_started(&self) -> DateTime {
        self.initial_time
    }
}
...
mod tests {
    ...
    fn test_time_estimation() {
        ...
        let next_timestamp = time_estimation.initial_time + Duration::seconds(60);
        time_estimation.update(next_timestamp, 0.1);

        assert_eq!(time_estimation.to_string(), "00:09:00 (± ?)");

        let next_timestamp = time_estimation.initial_time + Duration::seconds(30);
        time_estimation.update(next_timestamp, 0.2);

        assert_eq!(time_estimation.to_string(), "00:05:00 (± 00:03:00)");

        let next_timestamp = time_estimation.initial_time + Duration::seconds(200);
