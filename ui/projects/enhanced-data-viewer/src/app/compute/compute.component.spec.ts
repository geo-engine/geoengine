...
describe('ComputeComponent', () => {
    ...
    it('derives a bbox from the polygon overlay and allows histogram computation when the data is ready', async () => {
        ...
        const bbox = component.computationBbox();
        expect(bbox).toBeDefined();
        expect(bbox?.bbox.xmin).toBe(0);
        expect(bbox?.bbox.ymin).toBe(0);
        expect(bbox?.bbox.xmax).toBe(1);
        expect(bbox?.bbox.ymax).toBe(1);
        expect(bbox?.spatialReference.srsString).toBe('EPSG:4326');
